...
class WebHelper {
  ...
  Future<FileInfo> downloadFile(String url,
      {double width, Map<String, String> authHeaders, bool ignoreMemCache = false}) async {
    if (!_memCache.containsKey(url) || ignoreMemCache) {
      var completer = new Completer<FileInfo>();
      _downloadRemoteFile(url, width: width, authHeaders: authHeaders).then((cacheObject) {
  ...
  Future<FileInfo> _downloadRemoteFile(String url,
      {double width, Map<String, String> authHeaders}) async {
    return Future.sync(() async {
      ...
      success = await _handleHttpResponse(response, cacheObject, imageWidth: width);
  ...
  Future<bool> _handleHttpResponse(
      FileFetcherResponse response, CacheObject cacheObject, {double imageWidth}) async {
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      // resize the image to minimize the size of the rendering cache
      if (imageWidth != null) {
        ImageProperties properties = await FlutterNativeImage.getImageProperties(path);
        if (imageWidth < properties.width) {
          File thumb = await FlutterNativeImage.compressImage(path, quality: 100, 
            targetWidth: imageWidth.toInt(), 
            targetHeight: (properties.height * imageWidth / properties.width).round());
          await thumb.copy(path);
        }
      } 
