...
abstract class BaseCacheManager {
  ...
  Future<File> getSingleFile(String url, {double width, Map<String, String> headers}) async {
    var cacheFile = await getFileFromCache(url);
    if (cacheFile != null) {
      if (cacheFile.validTill.isBefore(DateTime.now())) {
        webHelper.downloadFile(url, width: width, authHeaders: headers);
      }
      return cacheFile.file;
    }
    try {
      var download = await webHelper.downloadFile(url, width: width, authHeaders: headers);
  ...
  Stream<FileInfo> getFile(String url, {double width, Map<String, String> headers}) async* {
    var cacheFile = await getFileFromCache(url);
    if (cacheFile != null) {
      yield cacheFile;
    }
    if (cacheFile == null || cacheFile.validTill.isBefore(DateTime.now())) {
      try {
        var webFile = await webHelper.downloadFile(url, width: width, authHeaders: headers);
        if (webFile != null) {
          yield webFile;
        }
      } catch (e) {
  ...
  Future<FileInfo> downloadFile(String url,
      {double width, Map<String, String> authHeaders, bool force = false}) async {
    return await webHelper.downloadFile(url, width: width,
